...
def test_sidecar_contract_requires_exact_sdk_and_compiler_versions() -> None:
    package = json.loads(
        (_repo_root() / offline_bundle.SIDECAR_PACKAGE_FILE).read_text(
            encoding="utf-8"
        )
    )
    ...
    assert contract["compiled_entrypoint"] == (
        "apps/cline-sidecar/dist/src/index.js"
    )
...
def test_frontend_checkout_and_build_inputs_include_the_sidecar(tmp_path: Path) -> None:
