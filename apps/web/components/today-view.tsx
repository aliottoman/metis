...
const KIND_ORDER: AttentionKind[] = ["run_approval", "customer_action", "customer_note", "tool_proposal", "memory", "asset_trust", "stale_source"];
...
function sections(feed: AttentionFeed | null, brief: MorningBrief | null): Section[] {
  ...
  const out: Section[] = [];
  if (feed.top.length) out.push({ id: "focus", label: "Start here", items: feed.top, batchable: false });
...
export function TodayView() {
  ...
  const refresh = useCallback(async (regenerate = false) => {
    setLoading(true);
    setError(null);
    const briefRequest = getMorningBrief(24, regenerate).then(setBrief).catch(() => undefined);
    try {
  ...
  useEffect(() => () => {
    ...
    setClip((current) => {
      if (current && current !== "loading") URL.revokeObjectURL(current.url);
      return null;
    });
  }, []);
  ...
  const headline = loading && !feed ? "Checking what's waiting" : total === 0 ? "You're all clear" : `${total} ${total === 1 ? "thing needs" : "things need"} you`;

  // Arrow keys walk the list; Enter opens; L defers; X selects; R brings back.
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    if (!rows.length || (event.target instanceof HTMLElement && event.target.tagName === "INPUT" && event.key !== "Escape")) return;
    ...
    switch (event.key) {
      ...
      case "Enter": if (row && !row.section.deferred) router.push(row.item.href || "/"); break;
      case "l": if (row && !row.section.deferred) void defer(row.item); break;
      case "r": if (row?.section.deferred) void restore(row.item); break;
      case "x": case " ": if (row?.section.batchable) { event.preventDefault(); toggle(row.item.key); } break;
      default: return;
    }
  };
  ...
  return (
    <div className="today">
        ...
        actions={
          <>
            {clip && clip !== "loading" ? (
              ...
              <button type="button" className="ui-btn" onClick={() => void listen()} disabled={clip === "loading"}>{clip === "loading" ? "Preparing…" : "Listen to the brief"}</button>
            )}
            <button type="button" className="ui-btn is-quiet" onClick={() => void refresh(true)} disabled={loading}>Refresh</button>
          </>
        }
      />

      {brief?.narrative ? <p className="today-brief">{brief.narrative}</p> : null}
      ...
      <div className="today-list" ref={list} tabIndex={groups.length ? 0 : -1} onKeyDown={onKeyDown} aria-label="What needs you" aria-keyshortcuts="ArrowUp ArrowDown Enter l x r">
        {groups.map((section) => (
          <section key={section.id} id={`today-${section.id}`} className="today-section">
            <header>
              ...
              {section.batchable ? (
                <button type="button" className="ui-btn is-quiet is-sm" onClick={() => {
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <li key={item.key} data-row={at} className={`today-row${cursor === at ? " is-cursor" : ""}${item.overdue ? " is-overdue" : ""}${selected.has(item.key) ? " is-selected" : ""}`} onPointerEnter={() => setCursor(at)}>
                      {section.batchable ? <input type="checkbox" checked={selected.has(item.key)} onChange={() => toggle(item.key)} aria-label={`Select ${item.title}`} /> : <span className={`ui-dot ${item.overdue ? "is-needs-review" : section.deferred ? "is-stopped" : "is-waiting"}`} aria-hidden="true" />}
                      ...
                      <div className="today-row-actions">
                        {section.deferred ? (
                          <button type="button" className="ui-btn is-quiet is-sm" disabled={busyKey === item.key} onClick={() => void restore(item)}>Bring back</button>
                        ) : (
                          <>
                            <a className="ui-btn is-sm" href={item.href || "/"}>Open</a>
                            <button type="button" className="ui-btn is-quiet is-sm" disabled={busyKey === item.key} title={`Ask again in ${DEFER_DAYS} days`} onClick={() => void defer(item)}>Later</button>
                          </>
                        )}
                      </div>
                    </li>
      ...
      {selected.size ? (
        <div className="today-batch" role="region" aria-label="Selected items">
          ...
          <button type="button" className="ui-btn is-primary is-sm" disabled={busyKey === "batch"} onClick={() => void runBatch("approve")}>Approve / complete</button>
          <button type="button" className="ui-btn is-sm" disabled={busyKey === "batch"} onClick={() => void runBatch("reject")}>Reject</button>
          <button type="button" className="ui-btn is-sm" disabled={busyKey === "batch"} onClick={() => void runBatch("defer")}>Later</button>
          <button type="button" className="ui-btn is-quiet is-sm" onClick={() => setSelected(new Set())}>Clear</button>
        </div>
      ) : null}
