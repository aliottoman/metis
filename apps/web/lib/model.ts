 ...
 * (Grok via OCI, Command A+ via Cohere) or a hosted Ollama model pinned on the
 * "local" lane. Cloud runs keep no on-device weights resident, so nothing about
 * them should demand a local launch. This is the one predicate the whole web
 * layer should branch on instead of inspecting the local session alone.
 */
export function isCloudActive(preference: ModelPreference | null | undefined): boolean {
  if (!preference) return false;
  if (preference.provider === "oci" || preference.provider === "cohere") return true;
 ...
 * model control: "Command A+" / "Grok" for the cloud providers, "Hosted · X"
 * for a hosted Ollama model, otherwise the resident local model. Null only when
 * nothing is active — no cloud pin and no local model ready — which is the sole
 * case that should ever read as "off".
 */
