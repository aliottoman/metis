...
def _validate_diagrams_render_v1(
    ...
    if spec is None:
        raise CodeProfileError(
            "diagrams-render-v1 requires an architecture spec in context"
        )
    try:
...
def _validate_diagrams_draw_v2(source: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    if spec is None:
        raise CodeProfileError(
            "diagrams-draw-v2 requires an architecture spec in context"
        )
    try:
...
def _validate_declarative_host_v1(
    source: str, context: dict[str, Any]
) -> dict[str, Any]:
    raise CodeProfileError(
...
def _validate_pure_python_authored_v1(
    source: str, context: dict[str, Any]
) -> dict[str, Any]:
