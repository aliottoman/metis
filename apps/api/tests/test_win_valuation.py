...
@pytest.mark.anyio
async def test_host_prices_the_model_quantities(
    ...
    win = await database.create_customer_win(
        account["id"],
        title="H100 cluster",
        brief="They deployed a 4xH100 GPU cluster.",
        services=["DAC"],
        dac_shape="",
        yearly_arr=None,
        won_at=None,
...
async def test_estimate_never_becomes_the_win_arr_until_accepted(
    ...
    assert (await database.get_customer_win(win["id"]))["yearly_arr"] == pytest.approx(
        87_600.0
    )
...
def test_rate_card_endpoints_round_trip(client: TestClient) -> None:
    ...
    seed = json.loads(
        (
            Path(__file__).resolve().parents[1] / "src/waqil_api/data/skus/rates.json"
        ).read_text(encoding="utf-8")
    )
    assert (
        next(item for item in seed["rates"] if item["part_number"] == "B98415")["value"]
        == 10.0
    )
