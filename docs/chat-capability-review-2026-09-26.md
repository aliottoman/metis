...
1. Auto now recognizes the reported “released recently” question and similar
   ...
   a linked source from one answer call.
...
4. The answer footer counts sources actually cited in the delivered answer.
   ...
   longer claims that a score check proves grounding.
...
5. **Project capability gate:** mocked sidecar and deterministic end-to-end
   tests pass, but they cannot demonstrate that real models consistently
   build projects. Run the existing live qualification funnel in
...
Podman-dependent case passed when rerun with Podman available. No live model or
project qualification run was performed for this review. A direct live lookup
with the exact failed question returned four pages, including Cline's own
[harness-upgrade article](https://cline.bot/blog/how-we-migrated-11-million-users-to-clines-biggest-harness-upgrade);
that checks retrieval, not the quality of a full generated answer. The
`fastapi-repair` project qualification preview resolved its four required
files and pinned ClineCore route without an inference call. The isolated
checkout has no Cline API key configured for a live build evaluation.
